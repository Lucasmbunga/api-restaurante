...
public class SaidaCaixaController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<SaidaCaixa>>> obterSaidasDiaria()throws RecursoNaoEncontradoException {
        return ResponseEntity.ok(saidaCaixaService.obterSaidasDiaria("/turnos/"));
    }

    @GetMapping("/{idTurno}")
    public ResponseEntity<ApiResponse<List<SaidaCaixa>>> obterSaidasPorTurno(@PathVariable Long idTurno)throws RecursoNaoEncontradoException {
