...
public class MesaService {
    ...
    public ApiResponse<Mesa> buscarMesaPorId(Long id,String path) throws NotFoundException {
        if(!mesaRepository.findById(id).isPresent()){
            throw new NotFoundException("Não foi encontrada uma mesa com id "+id);
        }
    ...
    public ApiResponse<Mesa> editarMesa(Long id,Mesa mesa,String path)throws NotFoundException {
        if(!mesaRepository.findById(id).isPresent()){
            throw new NotFoundException("Não foi encontrada uma mesa com id "+id);
        }
    ...
    public ApiResponse<Void> desocuparMesa(Long id,String path)throws NotFoundException {
        var mesa=mesaRepository.findById(id).orElseThrow(()->new NotFoundException("Mesa não encontrada."));
    ...
    public ApiResponse<Void> excluirMesa(Long id,String path)throws NotFoundException {
        if(mesaRepository.findById(id).isEmpty()){
            throw new NotFoundException("Não foi encontrada um mesa com id "+id);
        }
